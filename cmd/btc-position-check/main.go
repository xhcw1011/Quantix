// One-off: read-only check of BTCUSDT position/orders on credential-id=6/user-id=4
// (real money). Pure read-only — no order placement, no cancellation.
package main
...
func main() {
	...
	cred, err := store.GetCredentialByID(ctx, 6, 4)
	if err != nil {
		die("get cred: %v", err)
	}
