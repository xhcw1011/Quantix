...
// Dispatcher delivers a fired alert. Tiny so tests can fake it and the production
// adapter can wrap the existing notifier.
...
func NewNotifyDispatcher(n *notify.Notifier) *NotifyDispatcher { return &NotifyDispatcher{n: n} }
...
func (d *NotifyDispatcher) Send(title, msg string) error {
	...
	d.n.SystemAlert("RADAR", title+" — "+msg)
	return nil
}
