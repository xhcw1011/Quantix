...
- **Gate on/off timeline**: reuse `grid_gate_backtest.py::run_grid`'s embedded 3-mechanism state machine (hysteresis/cooldown/persistence) to get the actual gate state per bar. This is the same logic `volgate.go` runs in Go; reusing the Python port that already exists avoids a second hand-transcription that could silently drift from production behavior (one known divergence found in review: during warmup — no lagged score yet — the Python port treats the gate as OFF while `volgate.go` keeps it ON; doesn't affect this gauge's conclusion since warmup is ~100 bars out of 8640+ per run, but the equivalence claim above isn't unqualified).
