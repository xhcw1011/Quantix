...
- **Data fetch**: reuse `breakout_score.py`'s `get`/`fetch_klines` (Binance fapi, same source `grid_trend_switch.py` already uses). Symbols/intervals: BTC + ETH, 15m and 5m — the exact set TED was validated on, so results are directly comparable to the frozen 17–130× numbers.
